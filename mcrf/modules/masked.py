...
from . import get_transition_mask_mat_from_allowed
...
class MaskedCRF(nn.Module):
    ...
    def __init__(self, num_tags: int, constraints: List[Tuple], batch_first: bool = True,
                 masked_decoding: Optional[bool] = True, masked_training: Optional[bool] = True) -> None:
        if num_tags <= 0:
        ...
        self.trans_mask, self.start_trans_mask, self.end_trans_mask = \
            get_transition_mask_mat_from_allowed(num_tags, constraints, return_start_end_transitions=True)
    ...
    def forward(
        ...
        if self.masked_training:
            ...
            transitions, start_transitions, end_transitions = self.transitions, self.start_transitions, self.end_transitions

        # shape: (batch_size,)
        numerator = self._compute_score(transitions, start_transitions, end_transitions, logits, tags, mask)
        # shape: (batch_size,)
        denominator = self._compute_normalizer(transitions, start_transitions, end_transitions, logits, mask)
        # shape: (batch_size,)
    ...
    def decode(self, logits: torch.Tensor,
        ...
        return self._viterbi_decode(transitions, start_transitions, end_transitions, logits, mask)

    def get_min_mask(self):
        masked_transitions = torch.min(self.transitions, self.trans_mask)
        masked_start_transitions = torch.min(self.start_transitions, self.start_trans_mask)
    ...
    def _compute_score(
            self, transitions, start_transitions, end_transitions,
            emissions: torch.Tensor, tags: torch.LongTensor,
    ...
    def _compute_normalizer(
            self, transitions, start_transitions, end_transitions,
            emissions: torch.Tensor, mask: torch.ByteTensor) -> torch.Tensor:
        # emissions: (seq_length, batch_size, num_tags)
        # mask: (seq_length, batch_size)
    ...
    def _viterbi_decode(self, transitions, start_transitions, end_transitions,
